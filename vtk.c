...
int write_checkpoint(int iteration) {
    ...
    if (enable_comparison) {
        char tail[1024];
        sprintf(tail, "_%d.cmp", iteration);
        sprintf(filename, comp_file_name_base, tail);
...
int write_result() {
    char file_name[1024];
    if (enable_comparison) {
        sprintf(file_name, comp_file_name_base, ".cmp");
...
int write_vtk(char* filename) {
    ...
    for (int j = 0; j <= Y; j++) {
        for (int i = 0; i <= X; i++)
            if (enable_comparison)
                fprintf(f, "%.15f %.15f 0.000000000000000\n", E[i][j][0], E[i][j][1]);
            else
    ...
    for (int j = 0; j <= Y; j++) {
        for (int i = 0; i <= X; i++)
            if (enable_comparison)
                fprintf(f, "0.000000000000000 0.000000000000000 %.15f\n", B[i][j][2]);
            else
